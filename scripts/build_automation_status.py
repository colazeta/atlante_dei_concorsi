...
def main() -> None:
    ...
    freshness = freshness_status(source_age, classification_age, triage_age) if triage else freshness_status(source_age, classification_age)
    ...
    status = {
        ...
        'latest_counts': {**source_counts, **classification_counts, **triage_counts},
        'top_triage_exclusion_reasons': triage.get('top_exclusion_reasons', []) if triage else [],
        'warnings': warnings,
        'next_expected_step': 'Continue scheduled source-inventory, document-classification and relevance-triage refresh cycle.',
    }
