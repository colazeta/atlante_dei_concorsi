...
def main():
    ...
    for r in rows:
        ...
        idx.append({'university_id':uid,'university_name':r.get('university_name',''),'intake_pack_exists':'yes','inventory_exists':'yes','homepage_url':homepage,'recruitment_url':recruitment,'fetch_status':'pending_fetch','candidate_links_count':'0','requires_human_attention':'yes' if (r.get('requires_human_attention','').strip().lower() in {'true','yes'} or not recruitment) else 'no','blocking_status':(r.get('blocking_status') or 'awaiting_bounded_fetch'),'last_updated_utc':now})
        by.append({**idx[-1]})
