...
def main():
        ...
        if not source_url:
            ...
            try:
                ...
                for href,text in p.links:
                    if candidates_recorded>=args.max_links_per_page: break
                    ...
                    if args.depth==1 and same=='yes' and km=='yes' and followed < args.max_follow_links_per_university:
                ...
                for follow_url,_t in to_follow:
                    try:
                        ...
                        errors += 1
                log_lines.append(f'  - fetched: {source_url} status={status} depth={args.depth} follow_links={len(to_follow)} links_recorded={candidates_recorded}')
            except (HTTPError,URLError,TimeoutError,ValueError) as e:
    ...
    depth1_pages=sum(1 for r in read_csv(SOURCE_INDEX) if r.get('fetch_status')=='fetched') if SOURCE_INDEX.exists() else 0
    progress={'updated_at_utc':stamp,'run_mode':'bounded_approved_source_fetch','batch_size':len(picks),'depth':args.depth,'depth_1_pages_followed':depth1_pages if args.depth==1 else 0,'errors':errors,'notes':['Fetched only approved intake URLs; depth limited to 1; same-domain and keyword-relevant follow links only; no raw document capture.']}
    ...
    CAND_PROGRESS.write_text(json.dumps({'updated_at_utc':stamp,'status':'candidate_links_populated_for_batch','batch_size':len(picks),'errors':errors,'depth':args.depth,'unknown_links':unknown_links,'records_requiring_attention':unknown_links},indent=2)+'\n',encoding='utf-8')
